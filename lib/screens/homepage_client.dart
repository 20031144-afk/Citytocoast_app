...
class _HomePageScreenState extends State<HomePageScreen> {
  ...
  Future<void> _handleManualRefresh() async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Cleaning "Sitter X" & restoring seeds...')),
    );

    // 1. Delete garbage "Sitter 1", "Sitter 2" etc.
    await cleanupLegacySitters();

    // 2. Ensure "Chloe", "Benjamin" etc. exist (do NOT delete custom data)
    await seedSitters(overwrite: false);

    // 3. Reload
    await _loadFirestoreSitters();

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Done! Data should be correct now.')),
      );
    }
  }

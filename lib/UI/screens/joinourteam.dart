...
class _JoinOurTeamState extends State<JoinOurTeam> {
  ...
  void _submit() {
    ...
    final selectedWorkTypes = workTypes.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .join(', ');
    ...
    final selectedAreas = areas.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .join(', ');
    ...
    final selectedRequirements = requirements.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .join(', ');
    print('Requirements: $selectedRequirements');

    final selectedHear = hearAbout.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .join(', ');
